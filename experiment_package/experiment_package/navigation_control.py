...
class NavigationControl:
    ...
    def __init__(self, node):
        ...
        while not self.controller_server_cli.wait_for_service(timeout_sec=1.0):
            self.node.get_logger().warn('Service "/artbul/controller_server/set_parameters" not available, waiting again...')
        ...
        while not self.lifecycle_manager_nav_cli.wait_for_service(timeout_sec=1.0):
            self.node.get_logger().warn('Service "/artbul/lifecycle_manager_navigation/manage_nodes" not available, waiting again...')
        ...
        while not self.lifecycle_manager_loc_cli.wait_for_service(timeout_sec=1.0):
            self.node.get_logger().warn('Service "/artbul/lifecycle_manager_localization/manage_nodes" not available, waiting again...')
    ...
    def change_nav2_parameter(self, param_name, param_type, param_value):
        ...
        if self.future.result() is not None:
            ...
            self.node.get_logger().error(f'Service call failed for {param_name}')
    ...
    def cancel_navigation(self):
        if self.goal_handle:
            ...
            self.node.get_logger().warn('No active navigation goal to cancel.')
...
class NavigationControl:
    ...
    def __init__(self, node):
        ...
        while not self.controller_server_cli.wait_for_service(timeout_sec=1.0):
            self.node.get_logger().info('Service "/artbul/controller_server/set_parameters" not available, waiting again...')
        ...
        while not self.lifecycle_manager_nav_cli.wait_for_service(timeout_sec=1.0):
            self.node.get_logger().info('Service "/artbul/lifecycle_manager_navigation/manage_nodes" not available, waiting again...')
        ...
        while not self.lifecycle_manager_loc_cli.wait_for_service(timeout_sec=1.0):
            self.node.get_logger().info('Service "/artbul/lifecycle_manager_localization/manage_nodes" not available, waiting again...')
    ...
    def feedback_callback(self, feedback_msg):
        ...
        # self.node.get_logger().info(f"{self.feedback}")
    ...
    def change_nav2_parameter(self, param_name, param_type, param_value):
        ...
        if self.future.result() is not None:
            self.node.get_logger().info(f'Parameter {param_name} set successfully: {self.future.result().results[0].successful}')
            return True
        else:
            self.node.get_logger().info(f'Service call failed for {param_name}')
            return False
    ...
    def reset_navigation_stack(self):
        ...
        rclpy.spin_until_future_complete(self.node, self.future)
        if self.future.result() is not None:
            self.node.get_logger().info(f'Lifecycle of navigation was shutdown successfully, status: {self.future.result().success}')
        else:
            self.node.get_logger().error(f'Service lifecycle_manager_nav call failed')

        self.future = self.lifecycle_manager_loc_cli.call_async(lifecycle_manager_request)
        rclpy.spin_until_future_complete(self.node, self.future)
        if self.future.result() is not None:
            self.node.get_logger().info(f'Lifecycle of localization was shutdown successfully, status: {self.future.result().success}')
        else:
            self.node.get_logger().error(f'Service lifecycle_manager_loc call failed')
    ...
    def startup_navigation_stack(self):
        ...
        rclpy.spin_until_future_complete(self.node, self.future)
        if self.future.result() is not None:
            self.node.get_logger().info(f'Lifecycle of localization started successfully, status: {self.future.result().success}')
        else:
            self.node.get_logger().error(f'Service lifecycle_manager_loc call failed')
            return False
        self.future = self.lifecycle_manager_nav_cli.call_async(lifecycle_manager_request)
        rclpy.spin_until_future_complete(self.node, self.future)
        if self.future.result() is not None:
            self.node.get_logger().info(f'Lifecycle of navigation started successfully, status: {self.future.result().success}')
            return True
        else:
            self.node.get_logger().error(f'Service lifecycle_manager_nav call failed')       
            return False
    ...
    def update_robot_pose(self, pose):
        ...
        self.node.get_logger().info("Robot pose is updated.")
    ...
    def start_robot_navigation(self, pose):
        ...
        while self.robot_navigate:
            ram.append(round(psutil.virtual_memory().percent/ 100, 3))
            cpu.append(round(psutil.cpu_percent()/100, 3))
            if self.feedback:
                navigation_time = self.feedback.navigation_time.sec + self.feedback.navigation_time.nanosec / (10 ** 9)
                if navigation_time > nav_timeout or self.feedback.number_of_recoveries > 5:
                    canceled_task = True
    ...
    def cancel_navigation(self):
        if self.goal_handle:
            ...
            self.node.get_logger().info('Navigation goal cancellation requested.')
            self.goal_handle = None # Clear the handle after cancellation
        else:
            self.node.get_logger().info('No active navigation goal to cancel.')